...
class Root(tk.Tk):
   ...
   def __init__(self, *args, **kwargs):
      ...
      self.title(title)
      self.tk.call("wm", "iconphoto", self._w, tk.PhotoImage(file="images/boise-brewing-medium-logo.png"))
...
def run():
   ...
   nw_button = tk.Button(root, text="Increase", command=partial(inc_button_click, root.counter), padx=0, pady=0, image=pixel, compound="c", width=(width // 4), height=(height // 4))
   nw_button.grid(column=0, row=0)
   sw_button = tk.Button(root, text="Decrease", command=partial(dec_button_click, root.counter), padx=0, pady=0, image=pixel, compound="c", width=(width // 4), height=(height // 4))
   sw_button.grid(column=0, row=2)
   se_button = tk.Button(root, text="Decrease", command=partial(dec_button_click, root.counter), padx=0, pady=0, image=pixel, compound="c", width=(width // 4), height=(height // 4))
   se_button.grid(column=2, row =2)
   ne_button = tk.Button(root, text="Increase", command=partial(inc_button_click, root.counter), padx=0, pady=0, image=pixel, compound="c", width=(width // 4), height=(height // 4))
